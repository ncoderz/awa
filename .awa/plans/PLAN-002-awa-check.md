# PLAN-002: `awa check` — Traceability Chain Verification
...
`awa check` deterministically checks two dimensions:
...
Create `FEAT-CHK-check.md` — problem: traceability is honour-system; check makes it enforceable.
...
Create `REQ-CHK-check.md`:

- CHK-1: Validate command scans source files for traceability markers
- CHK-2: Validate command parses spec files for requirement IDs, AC IDs, property IDs, component names
- CHK-3: Validate command reports orphaned markers (code → non-existent spec)
- CHK-4: Validate command reports uncovered ACs (spec → no test)
- CHK-5: Validate command reports broken cross-references (DESIGN IMPLEMENTS → non-existent REQ ID)
- CHK-6: Validate command validates ID format conventions (configurable regex, default: `{CODE}-{n}`, `{CODE}-{n}.{p}`, `{CODE}-{n}_AC-{m}`, `{CODE}_P-{n}`)
- CHK-7: Validate command reports orphaned spec files (spec with CODE not referenced by any other spec or code) — WARNING level only, not error; specs for planned-but-not-yet-implemented features are expected
- CHK-8: Exit code 0 = clean, 1 = errors found
- CHK-9: Support `--format json` for CI consumption
- CHK-10: Support `--ignore` patterns for excluding paths
- CHK-11: Marker names are configurable via `.awa.toml` (default: `@awa-impl`, `@awa-test`, `@awa-component`)
- CHK-12: Spec file globs are configurable via `.awa.toml` (default: `.awa/specs/**/*.md`)
- CHK-13: Code file globs are configurable via `.awa.toml` (default: `src/**/*.{ts,js,tsx,jsx,py,go,rs,java,cs}`)
- CHK-14: ID pattern regex is configurable via `.awa.toml` (default: matches `{CODE}-{n}`, `{CODE}-{n}.{p}`, `{CODE}-{n}_AC-{m}`, `{CODE}_P-{n}`)
- CHK-15: Cross-reference patterns are configurable (default: `IMPLEMENTS:` and `VALIDATES:` in DESIGN files)
- CHK-16: All configuration has sensible defaults matching the bundled awa template workflow

### 3. DESIGN

Create `DESIGN-CHK-check.md`:

- CHK-ConfigLoader: Read check config from `[check]` section of `.awa.toml`
- CHK-SpecParser: Parse spec files matching configured globs to extract IDs (regex-based)
- CHK-MarkerScanner: Scan code files matching configured globs for configured marker names
- CHK-CodeSpecChecker: Match code markers against spec IDs, report orphaned markers and uncovered ACs
- CHK-SpecSpecChecker: Match cross-references between spec files (DESIGN IMPLEMENTS → REQ), validate ID format
- CHK-Reporter: Output results (text, JSON)
...
- Define `[check]` config schema in type definitions
- Restructure config loader to support nested TOML tables (`[check]` is a table, not a flat key — current loader only handles flat keys; requires new nested-table parsing logic, not just adding to `knownKeys`)
...
Update awa workflow templates (skills and prompts) to run `awa check` as a consistency check after changes:

- Update `awa-code` skill/prompt: after implementing code and tests, run `awa check` and fix any orphaned markers or uncovered ACs before considering the task complete
- Update `awa-requirements` skill/prompt: after writing/updating REQ specs, run `awa check --format json` to check ID format and cross-references
- Update `awa-design` skill/prompt: after writing/updating DESIGN specs, run `awa check` to verify IMPLEMENTS/VALIDATES references resolve to real REQ IDs
- Update `awa-refactor` skill/prompt: after refactoring, run `awa check` to ensure traceability markers weren't lost or broken
...
1. **Phase 1**: Code↔spec validation (CHK-1 through CHK-4, CHK-6, CHK-8) — core value
2. **Phase 2**: Spec↔spec validation (CHK-5, CHK-7) and JSON output (CHK-9)
3. **Phase 3**: Configurability (CHK-10 through CHK-16) and template integration (Section 7)
...
- `awa check` works out of the box with zero config for default awa workflow
