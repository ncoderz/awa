...
- [ ] Create `src/utils/update-check.ts` module
...
- [ ] Create `src/utils/update-check-cache.ts` module
...
- [ ] Add `printUpdateWarning(logger, result)` function (in `update-check.ts` or separate)
...
- [ ] Hook the update check into the CLI entry (`src/cli/index.ts`)
...
- [ ] Suppress the warning when `--json` or `--summary` flags are active
...
- [ ] Add optional `[update-check]` table to `.awa.toml` support in config loader
...
- [ ] Respect `NO_UPDATE_NOTIFIER` environment variable (community convention used by `update-notifier` and others) — skip check if set

### Testing

- [ ] Unit tests for version comparison logic (equal, patch bump, minor bump, major bump, pre-release)
- [ ] Unit tests for cache read/write and staleness logic (mock fs)
- [ ] Unit tests for `printUpdateWarning` output — minor/patch vs major version message variants
- [ ] Integration test: mock `fetch` to verify end-to-end flow (outdated, up-to-date, network error)
...
- [ ] Running `awa generate` (or any command) with an outdated version prints an update warning after command output
- [ ] Warning is suppressed with `--json`, `--summary`, or non-TTY stdout
- [ ] Check hits the network at most once per configured interval (default 1 day)
- [ ] Network failures are silent — CLI works identically when offline
- [ ] `NO_UPDATE_NOTIFIER=1` disables the check entirely
- [ ] All new code has unit tests; existing tests unaffected
