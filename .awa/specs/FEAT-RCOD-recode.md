...
When feature codes need to be merged or consolidated, the first mechanical step is rewriting all IDs from one code prefix to another. For example, merging the CHK feature into CLI requires every CHK ID (requirements, subrequirements, acceptance criteria, properties, component names, markers) to become a CLI ID with numbers that don't collide with existing CLI IDs.
...
- SOURCE CODE: The feature code being rewritten (e.g. CHK). All IDs with this prefix will change.
- TARGET CODE: The feature code to rewrite into (e.g. CLI). New IDs will use this prefix with numbers offset past the existing maximum.
...
- COMPONENT PREFIX REWRITING: Component names like CHK-MarkerScanner become CLI-MarkerScanner. The descriptive suffix is preserved; only the code prefix changes.

After recoding, the affected spec files still have their original filenames (e.g. REQ-CHK-check.md now contains CLI IDs). The user must rename files manually or use a merge command to complete the consolidation.
...
A developer decides to merge the CHK feature into CLI. They run `awa spec recode CHK CLI`. The tool finds the highest CLI requirement number (e.g. 15) and the highest CLI property number (e.g. 6), then rewrites every CHK ID: CHK-1 becomes CLI-16, its first acceptance criterion becomes CLI-16's first criterion, the first CHK property becomes CLI's seventh property, and CHK-MarkerScanner becomes CLI-MarkerScanner. All spec files, source code, and test markers are updated in place.

### Scenario 2: Dry Run Preview

Before committing to a recode, a developer runs `awa spec recode CHK CLI --dry-run` to see the full recode map and the list of affected files without modifying anything. This lets them verify the offset numbering looks correct and no unexpected files would be touched.

### Scenario 3: Target Has No Existing Requirements

A developer recodes CHK into a brand-new feature code NEW that has a REQ file but no existing requirements. The offset starts at 0, so CHK-1 becomes NEW-1, CHK-2 becomes NEW-2, and so on. Properties also start from 1.
