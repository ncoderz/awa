...
A source file contains `@awa-impl: FOO-1_AC-1` but `FOO-1_AC-1` doesn't exist in any spec file. `awa validate` reports an error and exits with code 1.
...
A DESIGN file says `IMPLEMENTS: CFG-99_AC-1` but `CFG-99_AC-1` doesn't exist in any REQ file. `awa validate` reports an error.
...
A spec file contains `FOO_1_AC-1` (underscore instead of hyphen). `awa validate` reports an ID format error.

### Scenario 8: Duplicate references

Two different source files both contain `@awa-impl: CFG-1_AC-1`. `awa validate` reports a warning about the duplicate implementation reference, helping developers spot accidental copy-paste or unclear ownership of acceptance criteria.
