...
- [ ] T-JSON-001 Create feature context document → .awa/specs/FEAT-JSON-json-output.md
- [ ] T-JSON-002 Create requirements specification → .awa/specs/REQ-JSON-json-output.md
- [ ] T-JSON-003 Create design specification → .awa/specs/DESIGN-JSON-json-output.md

## Phase 2: Setup

- [ ] T-JSON-004 Add --json and --summary flags to generate command → src/cli/index.ts
- [ ] T-JSON-005 [P] Add --json and --summary flags to diff command → src/cli/index.ts

## Phase 3: Foundation

- [ ] T-JSON-006 Define JSON output types (GenerationJSON, DiffJSON, counts) → src/types/index.ts
...
- [ ] T-JSON-010 [JSON-1] Implement JSON serializer for GenerationResult (actions array, counts) → src/core/json-output.ts
      IMPLEMENTS: JSON-1_AC-1, JSON-3_AC-1
- [ ] T-JSON-011 [P] [JSON-2] Implement JSON serializer for DiffResult (diffs array, counts) → src/core/json-output.ts
      IMPLEMENTS: JSON-2_AC-1, JSON-4_AC-1
- [ ] T-JSON-012 [P] [JSON-1] Test generate JSON output structure and validity → src/core/__tests__/json-output.test.ts
      TESTS: JSON-1_AC-1, JSON-3_AC-1
- [ ] T-JSON-013 [P] [JSON-2] Test diff JSON output structure and validity → src/core/__tests__/json-output.test.ts
...
- [ ] T-JSON-020 [JSON-6] Suppress @clack/prompts intro/outro and set Logger to silent when --json in generate → src/commands/generate.ts
      IMPLEMENTS: JSON-6_AC-1
- [ ] T-JSON-021 [P] [JSON-6] Suppress interactive output in diff command when --json → src/commands/diff.ts
      IMPLEMENTS: JSON-6_AC-1
- [ ] T-JSON-022 [JSON-7] Enforce --json implies --dry-run for generate command → src/commands/generate.ts
      IMPLEMENTS: JSON-7_AC-1
- [ ] T-JSON-023 [JSON-8] Write JSON to stdout; errors to stderr → src/core/json-output.ts
      IMPLEMENTS: JSON-8_AC-1
- [ ] T-JSON-024 [P] [JSON-6] Test --json suppresses interactive output and implies --dry-run → src/commands/__tests__/generate.test.ts
      TESTS: JSON-6_AC-1, JSON-7_AC-1
- [ ] T-JSON-025 [P] [JSON-8] Test stdout/stderr separation → src/commands/__tests__/generate.test.ts
      TESTS: JSON-8_AC-1
...
- [ ] T-JSON-030 [JSON-5] Implement summary formatter for generate and diff commands → src/core/json-output.ts
      IMPLEMENTS: JSON-5_AC-1
- [ ] T-JSON-031 [P] [JSON-5] Test summary output format → src/core/__tests__/json-output.test.ts
      TESTS: JSON-5_AC-1
