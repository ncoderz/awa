...
The `awa validate` command verifies the integrity of the traceability chain between code markers and spec files. It ensures that every `@awa-impl`, `@awa-test`, and `@awa-component` marker in source code corresponds to a real ID in the spec files, and that spec cross-references (IMPLEMENTS/VALIDATES) point to valid targets.
...
awa validate --format json

# Ignore specific paths
awa validate --ignore "test/fixtures/**" --ignore "examples/**"

# Custom config file
awa validate --config ./custom.toml
```
...
All validate settings can be specified in `.awa.toml` under the `[validate]` section. CLI flags override config values.
