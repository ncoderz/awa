...
import { ensureDir, getCacheDir, getTemplateDir, isDirectory, pathExists, readTextFile, walkDirectory, writeTextFile } from "../fs.js";

describe("FileSystemUtilities", () => {
  let testDir: string;

  beforeEach(async () => {
    testDir = join(tmpdir(), `zen-test-${Date.now()}`);
