...
<Tabs>
  <TabItem label="Global install">```bash npm install -g @ncoderz/awa ```</TabItem>
  <TabItem label="npx (no install)">```bash npx @ncoderz/awa init ```</TabItem>
</Tabs>
...
See the [CLI Reference](/reference/cli/) for full details on all spec management commands.
