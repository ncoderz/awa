...
- `--all-targets` and `--target` suppress interactive tool-feature prompting (non-interactive batch mode)
- `--all-targets` ignores the CLI positional `[output]` argument; `--target` allows CLI positional to override the target's output
- `--force`, `--dry-run`, and `--delete` apply globally to all targets when using `--all-targets`
- `--all-targets` requires at least one `[targets.*]` section; errors with `NO_TARGETS` otherwise
- `--target <name>` errors with `UNKNOWN_TARGET` if the name isn't defined
- `diff --all-targets` exit code: `0` if all targets match, `1` if any target has differences, `2` on error
